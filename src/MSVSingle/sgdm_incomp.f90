...
!         dzmax=min(dzmsgd(i,j,k),dzpsgd(i,j,k))
!         dymax=min(dymsgd(i,j,k),dypsgd(i,j,k))
!         dxmax=min(dxmsgd(i,j,k),dxpsgd(i,j,k))
!         dxyzsgd(i,j,k)=min(dxmax,dymax,dzmax)
         enddo
        enddo
       enddo
       do K=1,nz
        do j=1,ny
         ...
         dzmsgd(i,j,k)=(dxyzsgd(i,j,k)/dzmsgd(i,j,k))**(1.0/3.0)
         enddo
        enddo
       enddo
...
!_________________________________________________________________
      ...
      DO k=1,nz
       DO j=1,ny
        DO i=1,nx
         amut(i,j,k)=amu0*SQRT(amut(i,j,k))*dxyzsgd(i,j,k)
        END DO
       END DO
      END DO
      if(isgm.eq.2) then
...
!_________________________________________________________________
      ...
      DO k=1,nz
       DO j=1,ny
        DO i=1,nx
         amut(i,1,k)=amut(i,1,k)-7.5936*jbnm(i,j,k,5)*dtempp(i,j,k,2)/0.6
         if(amut(i,1,k).lt.0.0)amut(i,1,k)=0.0
        END DO
       END DO
      END DO
      endif
...
!_________________________________________________________________
      ...
      DO k=1,nz
       DO j=1,ny
        DO i=1,nx
         wx(i,j,k)=jbnp(i,j,k,5)*dcvel(i,j,k,8)-  &
                   jbnp(i,j,k,9)*dcvel(i,j,k,6)
         wy(i,j,k)=jbnp(i,j,k,9)*dcvel(i,j,k,3)-  &
                   jbnp(i,j,k,1)*dcvel(i,j,k,7)
         wz(i,j,k)=jbnp(i,j,k,1)*dcvel(i,j,k,4)-  &
                   jbnp(i,j,k,5)*dcvel(i,j,k,2)  
        ENDDO
       ENDDO
      ENDDO
      RETURN
