...
!-----------------------------------------------------------------------
      ...
      real, allocatable, dimension (:,:,:,:) :: dconcp,dconcm, dvel
      real, allocatable, dimension (:,:,:,:) :: dconczp,dconczm, dconc
      real, allocatable, dimension (:,:,:,:) :: dtempp,dtempm, dtemp
...
!-----------------------------------------------------------------------
      ...
      real, allocatable, dimension (:,:,:) :: conc,temp,pres,concz,pres0
...
!-----------------------------------------------------------------------
      ...
      character(32) :: inputgrid,inputfield, inputsield, inputtime
...
!-----------------------------------------------------------------------
      ...
      real :: reynolds,froude,prandtl,prandtl_t,gamma,mach, eckert
      real :: peclet, nahme
...
!-----------------------------------------------------------------------
      ...
      real ::  tiemp,thetas,phis,pi,minerro,lx,ly
      real :: dym,hm,phimod,delm
      INTEGER med,ea,eb,cont
      real,  allocatable, dimension (:,:) :: rmin, rsub,xx,yy,posdos
      real,  allocatable, dimension (:,:) :: erro,rtot,erroy,rtoty,rtotx

      end module mezclador

!----------------------------------------------------------------------
      module mix_para
!---------------------------------------------------------------------

      real :: RMAY, rini, atheta, aphi,lo,lasp,gat,gaa,intime, rossby
...
!-----------------------------------------------------------------------
      ...
      real, allocatable, dimension (:,:,:) :: impellermask

      end module maskvar


!-----------------------------------------------------------------------
      module impellermotion
!-----------------------------------------------------------------------

      use iso_c_binding

      TYPE, BIND(C) :: TRIANGLESTL
          REAL(C_DOUBLE) :: vrtx1(3)
          REAL(C_DOUBLE) :: vrtx2(3)
          REAL(C_DOUBLE) :: vrtx3(3)
          REAL(C_DOUBLE) :: n(3)
      END TYPE TRIANGLESTL

      TYPE(TRIANGLESTL), allocatable, dimension(:) :: triangleGeo

      end module impellermotion
