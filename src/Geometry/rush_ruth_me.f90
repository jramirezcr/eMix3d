...
!__________________________________________________________________
      ...
      implicit none
      integer neq, irk, i, j, itc, nmcit, k, no_imp, sub_step
      real :: ue, ve,  theta, Ro, dtime
      ...
      DO it_im = 1, 6
         ...
         call makeAspa(x, maskRush, theta, 0.25, 0.5, 1.05d0 - 0.1, & 
                        1.05d0 + 0.1,ancho, nx, ny, nz)
...
!----------------------------------------------------------------------------
      ...
      DO k =1,nz
       ...
       ((x(i,j,k,3).GE.(1.05d0 - 0.1)).AND.(x(i,j,k,3).LE.(1.05d0 + 0.1)))&
                 )&
                 )then
