...
!__________________________________________________________________
      ...
      DO it_im = 1, 6
         ...
         call makeAspa(x, maskRush, theta, 0.25, 0.5, 1.0 - 0.1, & 
                        1.0 + 0.1, ancho , nx, ny, nz)
         theta = theta + pi/3.d0 
      ENDDO

      CALL addMAskCil(maskRush, x, 0.0, 0.375, (1.0 - 0.5*ancho), &
                     (1.0 + 0.5*ancho), Nx, Ny, Nz) 
...
!----------------------------------------------------------------------------
      ...
      DO k =1,nz
         DO j =1,ny
            DO i =1,nx
               if((cflag(i,j).EQ.-1).AND.(x(i,j,k,3).GE.(1.0 - 0.1)).AND.&
                 (x(i,j,k,3).LE.(1.0 + 0.1)))then
              ...
              !   coefB1 = matrix(i,j,2)*amut(index1i(i,j),index1j(i,j),k)&
              !          + matrix(i,j,3)*amut(index2i(i,j),index2j(i,j),k)

              !   coefB2 = matrix(i,j,5)*amut(index1i(i,j),index1j(i,j),k)&
              !          + matrix(i,j,6)*amut(index2i(i,j),index2j(i,j),k)

              !   coefB3 = matrix(i,j,8)*amut(index1i(i,j),index1j(i,j),k) &
              !          + matrix(i,j,9)*amut(index2i(i,j),index2j(i,j),k)

              !   amut(i,j,k) = coefB1 + x(i,j,k,1)*coefB2 + x(i,j,k,2)*coefB3
