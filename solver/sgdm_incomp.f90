      ...
      real :: dxmin,dymin,dzmin
       ...
       do K=1,nz
        do j=1,ny
         ...
         dzmin=min(dzmsgd(i,j,k),dzpsgd(i,j,k))
         dymin=min(dymsgd(i,j,k),dypsgd(i,j,k))
         dxmin=min(dxmsgd(i,j,k),dxpsgd(i,j,k))

         deltamax = max(dxmax, dymax, dzmax, deltamax) 
         dmins(i,j,k) = min(dzmin, dymin, dxmin) 
...
!_________________________________________________________________
      ...
      DO k=1,nz
       DO j=1,ny
        DO i=1,nx
         amut(i,1,k)=amut(i,1,k)-7.5936*jbn(i,j,k,5)*dtemp(i,j,k,2)/0.6
